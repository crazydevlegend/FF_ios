...
public final class DropDown: UIView {
	...
	*/
	@objc public dynamic var cornerRadiusDD = DPDConstant.UI.CornerRadius {
		willSet {
	...
	*/
	@objc public dynamic var shadowColorDD = DPDConstant.UI.Shadow.Color {
	...
	Changing the shadow color automatically reloads the drop down.
	*/
	@objc public dynamic var shadowOffsetDD = DPDConstant.UI.Shadow.Offset {
	...
	*/
	@objc public dynamic var shadowOpacityDD = DPDConstant.UI.Shadow.Opacity {
	...
	*/
	@objc public dynamic var shadowRadiusDD = DPDConstant.UI.Shadow.Radius {
...
private extension DropDown {
	...
	func setupUI() {
		...
		tableViewContainer.layer.cornerRadius = cornerRadiusDD
		tableViewContainer.layer.shadowColor = shadowColorDD.cgColor
		tableViewContainer.layer.shadowOffset = shadowOffsetDD
		tableViewContainer.layer.shadowOpacity = shadowOpacityDD
		tableViewContainer.layer.shadowRadius = shadowRadiusDD

		tableView.backgroundColor = tableViewBackgroundColor
		tableView.separatorColor = separatorColor
		tableView.layer.cornerRadius = cornerRadiusDD
...
extension DropDown {
	...
	fileprivate func computeLayout() -> (x: CGFloat, y: CGFloat, width: CGFloat, offscreenHeight: CGFloat, visibleHeight: CGFloat, canBeDisplayed: Bool, Direction: Direction) {
		...
		barButtonItemCondition: if let anchorView = anchorView as? UIBarButtonItem {
			...
			let width = self.widthDD ?? fittingWidth()
	...
	fileprivate func computeLayoutBottomDisplay(window: UIWindow) -> ComputeLayoutTuple {
		...
		let width = self.widthDD ?? (anchorView?.plainView.bounds.width ?? fittingWidth()) - bottomOffset.x
	...
	fileprivate func computeLayoutForTopDisplay(window: UIWindow) -> ComputeLayoutTuple {
		...
		let width = self.widthDD ?? (anchorView?.plainView.bounds.width ?? fittingWidth()) - topOffset.x
